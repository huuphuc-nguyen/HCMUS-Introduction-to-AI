...
# Literal can be A, B, C,... or -A, -B, -C,...
# Logic expressions can be OR, AND, EQUIV,....
...
class pl_resolution:
    ...
    def print_output(self, details : bool = False, f : FileIO = None) -> None:
        ...
        for i in range(len(self.__details)):
            print(self.get_cnf_format_of(self.__details[i][0]) + ' res ' + self.get_cnf_format_of(self.__details[i][1]), file = f)
        '''